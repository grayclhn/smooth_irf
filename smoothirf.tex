...
\begin{equation*}
  \Psi_2(s) = e_{1,2} M J^s M^{-1} e_{1,2}'
  \begin{pmatrix*}[l]
    0 \\ 1
  \end{pmatrix*}.
\end{equation*}

We plot the \IRF s in the first two columns Figure~\ref{fig:3}. The
first column plots the standard \IRF s, using linear interpolation
...
often underestimated by the standard \IRF s and their timing is
...
\begin{figure}[t]
  ...
  \caption{Impulse Response Functions from Section~\ref{S3.1} example;
    ...
    plots standard \IRF s, the middle column plots our new method, and
...
we graph multiple perturbations of the \IRF s in the same panel, which
is a common way of representing uncertainty or set-identified
responses. To demonstrate this phenomenon, we generated 150
perterbuations of the \IRF s by adding independent $N(0, 0.15)$ noise
terms to each element of the \VAR\ coefficients in~\eqref{eq:12}, then
calculating and plotting the \IRF s as before. (The graphs use alpha
...
\begin{figure}[t]
  ...
  \caption{Impulse Response Functions from Section~\ref{S3.1} example;
    ...
    graphs plot the \IRF s from 150 independent draws of the
    coefficient matrices. The left column plots standard \IRF s, the
...
These graphs reveal that this method of constructing \IRF s is more
...
Now let $\Psi^*_1(t)$, $\Psi^*_2(t)$, and $\Psi^*_3(t)$ are the \IRF s
